...
weights_1.fill(params[0].data.numpy())
biases_1.fill(params[1].data.numpy())
...
weights_2.fill(params[2].data.numpy())
biases_2.fill(params[3].data.numpy())
...
weights_fc_1.fill(params[4].data.numpy())
biases_fc_1.fill(params[5].data.numpy())
...
weights_fc_2.fill(params[6].data.numpy())
biases_fc_2.fill(params[7].data.numpy())
...
weights_fc_3.fill(params[8].data.numpy())
biases_fc_3.fill(params[9].data.numpy())
fc_3 = Linear("output", "relu_fc_2", "weights_fc_3", "biases_fc_3")
layers.extend([weights_fc_3, biases_fc_3, fc_3])

net2 = Network(layers, dump_graph=True)
outputs = net2.execute()

diff = out.data.numpy() - outputs["output"]
print(np.max(diff))
if np.max(diff) > 0.5:
    print("elo")
    ...
    again = net2.execute()
    print("end")
print(np.allclose(out.data.numpy(), outputs["output"], atol=1e-05))
